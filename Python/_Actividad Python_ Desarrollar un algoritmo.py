...
# Find the index of `username` in `approved_users` and store it in `ind`
...
# Como approved_users y approved_devices están sincronizadas, el índice 3 también corresponde al dispositivo de sgilmore en approved_devices. Esto será muy útil en el siguiente paso.
...
# .index("sgilmore") → encuentra que sgilmore está en el índice 3
# approved_devices[3] → usa ese índice para obtener el dispositivo correspondiente "4n482ts"
...
if username in approved_users and approved_devices[approved_users.index(username)] == device_id:
    print("The username", username, "is approved to access the system.")
...
elif username in approved_users and approved_devices[approved_users.index(username)] != device_id:
    print("The user " + username + " is approved to access the system.")
    print("The user " + username + " does not have their assigned device.")
...
def login(username, device_id):
    ...
    if username in approved_users:
        ...
        # Inner conditional: check if device ID matches
        if approved_devices[approved_users.index(username)] == device_id:
            print("The user " + username + " is approved to access the system.")
            print("The user " + username + " has their assigned device.")
        else:
            print("The user " + username + " is approved to access the system.")
            print("The user " + username + " does not have their assigned device.")
...
# Call the function with different combinations to test it
login("sgilmore", "4n482ts")   # ✅ Approved user + correct device
print("---")
login("sgilmore", "8rp2k75")   # ⚠️ Approved user + wrong device
print("---")
login("jsmith", "1234567")     # ❌ User not approved
...
# The user sgilmore is approved to access the system.
# The user sgilmore has their assigned device.
# ---
# The user sgilmore is approved to access the system.
# The user sgilmore does not have their assigned device.
# ---
# The user jsmith is not approved to access the system.

# ## Conclusion
# 
# **What are your key takeaways from this lab?**

# Construimos un sistema de login usando listas paralelas, donde el índice de un usuario en approved_users corresponde a su dispositivo en approved_devices.
# Los conceptos clave fueron .append(), .remove(), .index(), condicionales anidados y funciones — todo combinado para verificar si un usuario y su dispositivo son válidos.
# La lección principal: dividir un problema grande en pasos pequeños hace que la solución final sea más fácil de entender y construir. 
